...
fn bench_memoized_inverse(c: &mut Criterion) {
    ...
    c.bench_function("memoized inverse", move |b| b.iter(|| pl.get_local_coords(&Point3::new(1002.0, -23.0, 282.))));

}

fn bench_eager_inverse(c: &mut Criterion) {
    let pl = dummy_zx_plane();
    c.bench_function("eager inverse", move |b| b.iter(|| pl.get_local_coords_eager(&Point3::new(1002.0, -23.0, 282.))));
}
...
fn bench_make_xy_plane(c: &mut Criterion) {
    ...
    c.bench_function("make xy plane", move |b| b.iter(|| geo::xy_plane(geo::RectBounds::new(100.0, 100.0))));
}

fn bench_make_zx_plane(c: &mut Criterion) {
    let bounds = geo::RectBounds::new(100.0, 100.0);

    c.bench_function("make zx plane", move |b| b.iter(|| geo::zx_plane(geo::RectBounds::new(100.0, 100.0))));
}
